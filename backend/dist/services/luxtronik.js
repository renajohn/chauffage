...
exports.initLuxtronik = initLuxtronik;
...
function getOperatingModeString(state) {
    switch (state) {
...
function initLuxtronik() {
    const { host, port } = config_1.config.pac;
...
function startPolling(onUpdate) {
    ...
    poll();
    pollTimer = setInterval(poll, config_1.config.polling.intervalMs);
}
function getCachedData() {
    return cachedData;
}
function writeParameter(param, value) {
    return new Promise((resolve, reject) => {
        ...
        connection.write(param, value, (err) => {
            if (err) {
...
//# sourceMappingURL=luxtronik.js.map