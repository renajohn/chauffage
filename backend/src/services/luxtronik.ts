...
function parseData(rawData: any): HeatPumpData {
  ...
  return {
    ...
    heatingCurve: {
      endPoint: parameters.heating_curve_end_point ?? parameters.heatingCurveEndPoint ?? 0,
      parallelOffset: parameters.heating_curve_parallel_offset ?? parameters.heatingCurveParallelOffset ?? 0,
      deltaReduction: parameters.deltaHeatingReduction ?? 0,
    },
  };
}
    ...
    connection.read((err: Error | null, data: any) => {
      if (err) {
        ...
        cachedData = {
          ...
          heatingCurve: { endPoint: 0, parallelOffset: 0, deltaReduction: 0 },
        };
        onUpdate(cachedData);
        return;
      }
